document.addEventListener('DOMContentLoaded', () => {
  ...
  function render() {
    ...
    root.innerHTML = `
      <div class="toolbar">
        <div class="toolbar__left"><input id="search-input" class="input" style="width:280px" placeholder="搜尋文章標題"></div>
        <div class="toolbar__right"><a class="btn btn--primary" href="./ai-generate.html">新增文章</a></div>
      </div>
      <div class="card"><div class="card__body"><div class="table-wrap"><table>
        <thead><tr><th>標題</th><th>狀態</th><th>分類</th><th>PV</th><th>Leads</th><th>更新時間</th><th>操作</th></tr></thead>
    ...
    const drawRows = (list) => {
      tbody.innerHTML = list.map(article => {
        const analytics = ArticleStore.getArticleAnalytics(article.id);
        return `<tr>
          <td><strong>${article.title}</strong><div class="inline-meta"><span>slug: ${article.slug}</span></div></td>
          ...
          <td>
            <div class="table-actions">
              ...
              <button class="btn btn--primary" data-action="publish" data-id="${article.id}">${article.status === 'published' ? '下架' : '發布'}</button>
    ...
    document.getElementById('search-input').addEventListener('input', (e) => {
      ...
      const filtered = articles.filter(item => item.title.toLowerCase().includes(keyword) || (item.summary || '').toLowerCase().includes(keyword));
      drawRows(filtered);
    });
    ...
    tbody.addEventListener('click', (e) => {
      ...
      if (action === 'publish') {
        ArticleStore.updateArticle(id, { status: article.status === 'published' ? 'draft' : 'published' });
        return render();
      }
      if (action === 'delete') {
  ...
  function viewArticle(article) {
    ...
    AdminCommon.openModal({
      ...
      <div class="grid grid--2">
        <div><strong>摘要</strong><p>${article.summary || '-'}</p></div>
        <div><strong>SEO</strong><p>SEO Title：${article.seoTitle || '-'}<br>SEO Description：${article.seoDescription || '-'}</p></div>
      </div>
      <div class="inline-meta" style="margin:12px 0 16px;"><span>PV：${analytics.pv}</span><span>Leads：${analytics.leads}</span><span>轉換率：${analytics.conversionRate}%</span></div>
      <div style="line-height:1.85;color:#334155">${article.content}</div>
      <div style="margin-top:18px"><a class="btn btn--line" href="../article.html?slug=${article.slug}" target="_blank">前台預覽</a></div>`
    });
  }
  ...
  function editArticle(article) {
    AdminCommon.openModal({
      ...
      <form id="edit-form" class="form-grid">
        ...
        <div><label>狀態</label><select class="select" name="status"><option value="draft" ${article.status==='draft'?'selected':''}>草稿</option><option value="published" ${article.status==='published'?'selected':''}>已發布</option></select></div>
        <div style="display:flex;align-items:end"><button class="btn btn--primary" type="submit">儲存變更</button></div>
      </form>`
    });
